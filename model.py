...
class Encoder(tf.keras.layers.Layer):
    ...
    def __init__(self,
        ...
        self.out_conv = tf.keras.layers.Conv2D(filters=nz,
                                      kernel_size=4,
                                      padding='valid')
...
class GANRunner:
    ...
    def save_best(self):
        self.G.save_weights(self.save_path + 'bestG')
        self.D.save_weights(self.save_path + 'bestD')

    def load_best(self):
        self.G.load_weights(self.save_path + 'bestG')
        self.D.load_weights(self.save_path + 'bestD')
...
class GANomaly(GANRunner):
    def __init__(self,
        ...
        super(GANomaly, self).__init__(self.G, self.D, best_state_key='roc_auc', best_state_policy=max,
    ...
    def train_step(self, x, y):
        ...
        if d_loss < 1e-5:
            ...
            logging.info('re-init D, cost: {:.4f} secs'.format(time.time()-st))
    ...
    def g_loss(self):
        ...
        g_loss = self.err_g_adv * self.opt.w_adv + self.err_g_con * self.opt.w_con + self.err_g_enc * self.opt.w_enc
        return g_loss
